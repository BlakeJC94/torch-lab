...
def main():
    ...
    output_dir = DATA_PROCESSED_DIR / "multiclass_one_hot"
    ...
    ann[VOTE_NAMES] = classes

    # For each patient, give category based on which annotations are present
    df = ann[["patient_id", *VOTE_NAMES]].groupby("patient_id").agg("any").astype(float)

    # Mask overrepresented patients
    mask_patients_overrep = (
        ann.groupby("patient_id").size().sort_values() > len(ann) * 0.001
    )
    ...
    for i, (train_index, val_index) in enumerate(skf.split(X, y)):
        train_ann = ann[ann["patient_id"].isin(X[train_index])]
        train_ann.to_csv(output_dir / f"train_{i}.csv", index=False)

        val_ann = ann[ann["patient_id"].isin(X[val_index])]
